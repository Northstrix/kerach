...
export function ColorPicker({
  ...
  // -----------------------------------------------------------------------
  // FIX START: Handle Achromatic Colors (White/Black/Gray)
  // -----------------------------------------------------------------------
  React.useEffect(() => {
    if (value) {
      const newHsv = getColorAsHsva(value);

      // When Saturation is 0 (Achromatic), the Hue is mathematically undefined 
      // and usually resets to 0. If the current state also has S=0, we should 
      // preserve the User's current Hue to prevent the slider from snapping back.
      if (newHsv.s === 0 && colorHsv.s === 0) {
        newHsv.h = colorHsv.h;
      }
  ...
  return (
    <div
      className={cn(
        'w-full max-w-[348px] space-y-4 rounded-lg border border-border bg-black p-4 text-white',
        className,
      )}
      style={
        {
    ...
    >
      <Saturation
        hsva={colorHsv}
        onChange={(newColor) => handleValueChange({ ...colorHsv, ...newColor })}
        style={{
          width: '100%',
          height: 'auto',
          aspectRatio: '2/1',
...
// ---------------------------------------------------
type ContrastRatioProps = { color: HsvaColor };
...
export function ContrastRatioInfo({ color }: ContrastRatioProps) {
  ...
  React.useEffect(() => {
    ...
    const blackLum = 0;
    const ratio =
      (Math.max(luminance, blackLum) + 0.05) /
      (Math.min(luminance, blackLum) + 0.05);
...
function HexColorInput({
  ...
  React.useEffect(() => {
    if (!isFocused) setInputValue(color);
  }, [color, isFocused]);
...
function ObjectColorInput({
  ...
  const [inputValues, setInputValues] = React.useState(() =>
    Object.fromEntries(
      keys.map((key) => [key, String((color as any)[key])]),
    ),
  );
  ...
  React.useEffect(() => {
    ...
    if (changed) setInputValues(newValues);
  ...
  const handleInputChange = (key: string, value: string) => {
    ...
    if (allValid) {
      ...
      for (const k of keys) updatedColor[k] = Number(newStringValues[k]);
      if ('a' in color) updatedColor.a = color.a;
  ...
  return (
    <div className="flex -mt-px">
      {keys.map((key, index) => {
        ...
        return (
          <div key={key} className="relative flex-1 min-w-0 focus-within:z-10">
            <Input
              className={cn(
                'peer shadow-none',
                index > 0 && '-ms-px',
                index === 0 && 'rounded-r-none',
                index === keys.length - 1 && 'rounded-l-none',
