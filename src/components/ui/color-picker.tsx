...
export function ColorPicker({
  ...
  return (
    <div
      className={cn(
        'w-full max-w-[348px] space-y-4 rounded-lg border border-border bg-popover p-4 text-popover-foreground',
        className
      )}
      style={
        {
      ...
      {!hideDefaultSwatches && (
        <>
          ...
          <div className="flex flex-wrap justify-start gap-2">
            {[
              '#00A2FA', '#A020F0', '#2CADF6', '#6366F1', '#D946EF', '#F97316',
              '#FBBF24', '#A3E635', '#22C55E', '#EF4444', ...swatches,
            ]
              ...
              .map((color) => (
                <button
                  ...
                  className="size-6 cursor-pointer rounded-md border border-[#242424] bg-[var(--swatch-color)] transition-all hover:scale-110 focus:outline-none"
                />
              ))}
          </div>
        </>
...
export function ContrastRatioInfo({ color }: ContrastRatioProps) {
  ...
  const ValidationBadge = ({
    ...
    <Badge
      ...
      className={cn(
        ...
        ratio >= ratioLimit && 'border-transparent bg-primary/20 text-primary',
        className
      )}
      {...props}
    >
  ...
  return (
    <div className="flex items-center gap-4">
      <div
        className="flex size-10 items-center justify-center rounded-lg shadow-sm ring-1 ring-inset ring-black/10"
        style={{ backgroundColor: hsvaToHex(color) }}
      >
        <span className="font-medium text-white mix-blend-difference">A</span>
      </div>
      <div className="flex-grow">
        <span className="text-sm font-medium text-muted-foreground">Contrast Ratio (Black)</span>
...
function HexColorInput({
  ...
  return (
    <Input
      className={cn('flex uppercase font-mono', isInvalid && 'border-red-500 focus-visible:ring-red-500')}
      value={inputValue}
...
function ObjectColorInput({ color, onChange, onInvalid }: ObjectColorInputProps) {
  ...
  const handleInputChange = (key: string, value: string) => {
    ...
    const allValid = keys.every((k) => {
      ...
      return newStringValues[k].trim() !== '' && !isNaN(numVal) && validate(k, numVal);
    });
    onInvalid(!allValid);
    if (allValid) {
  ...
  return (
    <div className="-mt-px flex">
      {keys.map((key, index) => {
        ...
        const isInvalid =
          isFocused === key &&
          (inputValues[key].trim() === '' || isNaN(numValue) || !validate(key, numValue));
        return (
          <div key={key} className="relative min-w-0 flex-1 focus-within:z-10">
            <Input
              className={cn(
                'peer shadow-none font-mono',
                index > 0 && '-ms-px',
                index === 0 && 'rounded-e-none',
                index === keys.length - 1 && 'rounded-s-none',
